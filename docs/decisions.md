...
- Thinking: Sonnet runs with thinking disabled; Gemini Flash cannot be fully switched off the same way, so it runs at `thinking_level: LOW`, the lowest level this model accepts (`MINIMAL` returned a 400 on the first smoke run), and its thinking tokens bill as output and are counted there.
- Caching: Claude's library prefix is cached explicitly with `cache_control` (a write at 1.25x, then reads at 0.1x). Gemini caches implicitly: a repeated prefix is discounted when the service hits and nothing is charged for writes, with no guarantee. The run records the cached tokens Gemini reports, so the cost is what was billed, and the estimate prints the no-hit worst case before any call. On the 2026-09-26 held-out run the implicit cache hit on 20 of 693 turns, so Gemini paid full input price for the library on almost every turn. `--gemini-cache explicit` instead creates one cache for the run's library (`client.caches.create`), names it on every call (`cached_content`), recreates it once if it expires, and deletes it at the end; its creation and hourly storage are reported under the table, apart from the per-call cost columns.
...
- Overload: the first Gemini smoke run on 2026-09-26 took 9.7 s on its first call, then got a 503 ("high demand") that outlasted the SDK's default four quick retries. The SDK's retries are now off and `core/llm.py` waits out 429 and 5xx answers itself (5, 10, 20, 40, 60, 60 s with jitter). Each wait is counted on the record (`infra_retries`) and left out of the latency, which is the successful attempt's alone. The results table says how many there were. A call that still fails stops the run; the on-disk response cache keeps every finished call, so rerunning the same command resumes without paying twice.
- Vertex AI: with `GOOGLE_GENAI_USE_VERTEXAI=true`, `GOOGLE_CLOUD_PROJECT` and `GOOGLE_CLOUD_LOCATION` set, and Application Default Credentials (`gcloud auth application-default login`), the same google-genai client calls the same model through Vertex AI. It bills to the Cloud billing account, which is where Samie's Google Developer Program credits apply. Each record says which backend served it (`gemini-api` or `vertex`), and the two are cached apart. The run is priced at the same unconfirmed list rates; Vertex's own price for the model has to be checked the same way before quoting.
